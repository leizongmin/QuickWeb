...
exports.extend = function (res, conf) {
  ...
  if (conf['enable gzip'] === true) {
    res.writeHead = warp.gzip_writeHead;
    res.write = warp.gzip_write;
    res.end = warp.gzip_end;
    res._qw_output = [];
    res._qw_output_length = 0;
  }
  else {
    res.writeHead = warp.writeHead;     // 新的writeHead方法
    res.end = warp.end; 
  }
...
warp.gzip_writeHead = function () {
  if (this._qw_accept_gzip !== false)
    this.setHeader('Content-Encoding', 'gzip');
  warp.writeHead.apply(this, arguments);
}
warp.gzip_write = function (data) {
...
methods.sendJSON = function (data, writeOnly) {
  try {
    ...
    if (writeOnly === true)
      ...
      this.end(out);
    debug('sendJSON length=' + out.length);
  }
  catch (err) {
...
methods.sendFile = function (filename, writeOnly) {
  try {
    ...
    filecache.readFile(filename, function (err, data) {
      if (err)
        methods.sendError.bind(self)(500, err.stack);
      else {
        // 输出文件内容
        if (self._qw_writeHeaded !== true)
    ...
    filecache.stat(filename, function (err, stats) {
      if (err) {
        ...
        if (isModified) {
          ...
          methods.sendFile.bind(self)(filename);
        }
        // 发送304
        else {
