...
if (process.env.QUICKWEB_DEBUG && /cluster/.test(process.env.QUICKWEB_DEBUG))
  if (cluster.isMaster) {
    ...
    debug('worker ' + process.pid + ' online.');
  }
else
  debug = function() { };
...
var x = util.inherits(PowerCluster, events.EventEmitter);
...
if (cluster.isMaster) {
   ...
   * @return {Worker}
   */
  PowerCluster.prototype.fork = function () {
    var self = this;
    ...
    // worker进程结束
    w.on('exit', function () {
      debug('worker ' + w.pid + ' exit.');
      delete self.workers[w.pid];
      self.emit('death', w);
    });
  ...
  PowerCluster.prototype.onWorkerMessage = function (pid, msg) {
    ...
    // worker online
    if (msg.cmd == 'online') {
      debug('worker online message');
      return;
    }
    
    // 广播消息
    if (msg.broadcast) {
      debug('broadcast from ' + pid);
  ...
  PowerCluster.prototype.send = function (pid, msg, from) {
    ...
    if (isNaN(from))
      from = 0;
    debug('send message to worker ' + pid);
  ...
  PowerCluster.prototype.broadcast = function (msg, from) {
    ...
    debug('broadcast message to ' + ret + ' workers.');
    return ret;
  }
}
...
if (cluster.isWorker) {
  ...
  PowerCluster.prototype.onMasterMessage = function (msg) {
    // 广播消息
    if (msg.broadcast) {
      debug('broadcast from ' + (msg.pid ? msg.pid : 'master'));
      this.emit('broadcast', msg.pid, msg.broadcast);
    }
    // 普通消息
    else if (msg.message) {
      debug('message from ' + (msg.pid ? msg.pid : 'master'));
      this.emit('message', msg.pid, msg.message);
    }
    // 无法识别
    else {
      debug('unknow message from ' + msg.pid);
