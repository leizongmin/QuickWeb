 ...
 * @return {bool}
 */
Route.prototype.add = function (path, handle) {
  ...
  if (p.path instanceof RegExp)
    ...
    this.staticTable[p.path] = handle;
  return true;
}
 ...
 * @return {object} 包含 index, handle, value
 */
Route.prototype.query = function (url, index) {
  // 先检查是否在 staticTable 中，如果没有在，再逐个判断 regexpTable
  if (url in this.staticTable)
    return {index: 0, handle: this.staticTable[url], value: null};
  ...
  for (var i = index, n = this.regexpTable.length; i < n; i++) {
    ...
    if (Array.isArray(r.names)) {
      for (var j = 0, nlen = r.names.length; j < nlen; j++)
        ret.value[r.names[j]] = pv[j + 1];
    }
    else {
      ret.value = null;
    }
