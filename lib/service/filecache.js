...
var clearCache = function () {
  ...
  debug('clear file cache start...' + min);
  
  for (var i in exports.cache) {
    var file = exports.cache[i];
    if (file.timestamp.data <= min) {
      delete exports.cache[i].data;
      delete exports.cache[i].timestamp.data;
      debug('clear file cache ' + i);
    }
    if (file.timestamp.stats <= min) {
      delete exports.cache[i].stats;
      delete exports.cache[i].timestamp.stats;
      debug('clear stats cache ' + i);
    }
  }
  
  setTimeout(clearCache, exports.expire / 2);
}
setTimeout(clearCache, exports.expire / 2);
