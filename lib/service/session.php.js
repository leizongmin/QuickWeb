...
if (process.env.QUICKWEB_DEBUG && /session/.test(process.env.QUICKWEB_DEBUG))
  debug = function(x) { console.error('Session.php: %s', x); };
else
  debug = function() { };
  ...
  filecache.readFile(filename, 'utf8', function (err, data) {
    ...
    if (err) {
      ...
      try {
        ...
        // 解析session文件出错
        debug('parse session file ' + filename + ' error: ' + err.stack);
        callback(err);
      }
    }
  });
...
exports.set = function (sessid, conf, data, callback) {
  ...
  try {
    ...
    // 序列化时出错
    debug('stringify session file ' + filename + ' error: ' + err.stack);
    callback(err);
  }
}
...
exports.destory = function (sessid, conf, callback) {
  ...
  try {
    ...
    debug('destory session file ' + filename + ' error: ' + err.stack);
    callback(err);
  }
}
