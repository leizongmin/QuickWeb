...
var ProcessMonitor = function (options) {
  ...
  debug('new Monitor: cycle=' + options.cycle + 'ms, count=' + options.count);
...
ProcessMonitor.prototype._onCycle = function () {
  ...
  debug('get process stat PID=' + this.pid.join());
  processStat(this.pid, function (err, data) {
    if (err) {
      debug('get process stat error: ' + err.stack);
      self.emit('error', err);
    }
    else {
      var timestamp = new Date().getTime();
      debug('get process stat at ' + timestamp);
...
ProcessMonitor.prototype.reset = function (cycle) {
  debug('reset process monitor, cycle=' + cycle);
  if (!isNaN(cycle))
...
ProcessMonitor.prototype.start = function () {
  ...
  debug('start process monitor, tid=' + this._watchTid);
}
